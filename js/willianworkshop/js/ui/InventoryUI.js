...
class InventoryUI {
    constructor() {
        this.els = {
            ...
            summaryContainer: document.getElementById('inventario-summary'),
    ...
    renderSummary(products = [], totalProducts = 0) {
        if (!this.els.summaryContainer) return;

        const list = Array.isArray(products) ? products : [];
        const totalVisible = list.length;
        const stockBajo = list.filter(p => p.stockReal > 0 && p.stockReal <= (p.stockMinimo || 0)).length;
        const stockCritico = list.filter(p => p.stockReal <= 0).length;
        const valorEstimado = list.reduce((sum, p) => sum + ((p.stockReal || 0) * (p.costo || 0)), 0);
        ...
        this.els.summaryContainer.innerHTML = `
            <div class="inventory-summary-card">
                <span class="inventory-summary-label">Productos</span>
                <strong>${totalVisible} / ${totalProducts}</strong>
            </div>
            <div class="inventory-summary-card">
                <span class="inventory-summary-label">Stock bajo</span>
                <strong class="summary-warning">${stockBajo}</strong>
            </div>
            <div class="inventory-summary-card">
                <span class="inventory-summary-label">Stock crítico</span>
                <strong class="summary-danger">${stockCritico}</strong>
            </div>
            <div class="inventory-summary-card">
                <span class="inventory-summary-label">Valor estimado</span>
                <strong class="summary-success">${formatCurrency(valorEstimado)}</strong>
            </div>
        `;
    }
